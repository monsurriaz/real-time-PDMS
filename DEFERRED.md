...
## M9.9 — booking form density + saved-address autofill

| Item | Notes |
|---|---|
| **A saved address's `zone` and `contactPhone` are optional on the shared `SavedAddress` type, even though adding/editing one still requires both** | Deliberate: a document saved before this session added `lastUsedAt`/`point` (or, further back, one from before `zone`/`contactPhone` existed on the model at all) can genuinely lack either. Widening the OUTPUT type rather than the input one is the same split `agentSelfSchema` and others already draw between "what the server accepts" and "what a `.lean()` read can honestly return." |
| **The pick-up point-reuse match check compares raw fields (`line1`/`area`/`zone`/`city` equality), not `geocode.ts`'s own `normaliseAddress`** | Considered reusing the address-cache's normalisation for consistency, but this comparison answers a different question: not "does this text describe the same place as some other text", but "is this literally the saved record the client says it is, unedited." Exact field equality is both simpler and more conservative — it never treats two different edits as the same address the way a looser normalisation theoretically could. |
| **`lastUsedAt` only stamps on a real booking that still matches the saved address verbatim** — not on `/quote`, and not when the submitted pick-up has been edited since a saved address was selected | The same `matched` flag `resolvePickupGeo` uses to decide whether to reuse a stored point also decides this, so the two can never disagree about what counts as "used this saved address." Verified live: a booking with an edited `line1` left the saved record's `lastUsedAt` exactly where an earlier, matching booking had put it. |
| **Recent recipients are capped at 8, scanning the most recent 100 parcels to find them** | A plain `ParcelModel.find()` (no aggregation, so the existing customer-ownership roleScope rule applies unchanged), deduplicated in JavaScript by name+phone+address, newest occurrence first. 100 is generous headroom for a customer who mostly reuses a handful of recipients; it costs one bounded query, not a second one per distinct recipient. |
| **The "save this pick-up address" offer appears only for a booking that did NOT use a saved address verbatim** — never for one that did, and it interrupts the post-booking redirect with one extra screen rather than saving silently | Matches the brief's "never saved without an explicit action" literally: an address already saved has nothing new to offer saving, and an edited-since-selected one is functionally a fresh address a customer might want under its own entry. The label defaults to the typed area name (editable later via the profile tab's own Edit) rather than prompting for one inline, keeping the offer to one tap either way. |
| **The progressive price panel reads `GET /pricing/example` (any authenticated role) rather than `POST /pricing/preview` (admin-only)** | `/preview` exists for the admin editor's unsaved draft rates and is deliberately not open to a customer; `/example` already computes the identical pure formula against the SAVED config, gated only by `requireAuth`, which is exactly what a customer choosing a weight/zone before addresses are geocoded needs. Pinned to `distanceKm: 0`; the panel reads only `zoneBase` and the weight tier from that response and never `distanceCost`/`total`, which a 0km distance would misreport. |
...
|---|---|---|
| **Saved addresses wired into booking's autofill** — pick-up prefills the most recently used saved address (falling back to the first), other saved addresses appear as chips to switch between, drop-off recalls the customer's own past recipients (derived from `Parcel.drop`, deduped, capped at 8, scoped through the existing roleScope rule), sender defaults from the profile and is overridden by a saved address's own contact when set, a saved address's first-resolved point is cached on it so a later autofilled booking skips Nominatim entirely, and a manually-typed pick-up gets a one-tap "save this address" offer after a successful booking. The booking form itself was also re-grouped onto fewer rows (pick-up/drop-off's road+area+zone on one row, name+phone on the next) to make room. | M9.9 | — |
